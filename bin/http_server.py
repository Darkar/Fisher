...
class Rod(BaseHTTPRequestHandler):
    def __init__(self, url, uagent, file):
        self.url = url
    ...
    def SaveTo(self, data, type="xml"):
        dt = datetime.now().strftime("date=\"%d/%m/%y\" time=\"%H:%M:%S\"")
        if type == "xml":
            with open(f"{self.file}.{type}", "a") as OutputFile:
                xml = dict2xml.dict2xml(data, wrap='domain', indent='   ').replace("<domain>", f"<domain name=\"{urlparse(self.url).netloc}\" {dt}>")
